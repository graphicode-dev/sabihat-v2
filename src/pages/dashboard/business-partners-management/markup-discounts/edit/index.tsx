...
function MarkUpEditPage() {
    const fetchedData = {
        ...
        visitType: "2",
        markupDiscount: "2",
        markupDiscountType: "2",
        markupDiscountValue: "2",
    ...
    const [selectedMarkupDiscount, setSelectedMarkupDiscount] = useState<
        string | null
    >(null);
    ...
    const { control, handleSubmit, reset, formState } = useForm<MarkUp>({
        ...
        defaultValues: {
            ...
            markupDiscount: fetchedData.markupDiscount,
            markupDiscountType: fetchedData.markupDiscountType,
            markupDiscountValue: fetchedData.markupDiscountValue,
    ...
    const onSubmit = async (formData: MarkUp) => {
        ...
        try {
            ...
            if (
                dirtyFields(formState).includes("markupDiscount") &&
                formData.markupDiscount
            ) {
                apiFormData.append("markupDiscount", formData.markupDiscount);
            }
            if (
                dirtyFields(formState).includes("markupDiscountType") &&
            ...
            if (
                dirtyFields(formState).includes("markupDiscountValue") &&
    ...
    useEffect(() => {
        if (fetchedData) {
            ...
            setSelectedMarkupDiscount(fetchedData.markupDiscount);
            setSelectedMarkupDiscountType(fetchedData.markupDiscountType);
            setSelectedMarkupDiscountValue(fetchedData.markupDiscountValue);
        }
    }, [reset]);
